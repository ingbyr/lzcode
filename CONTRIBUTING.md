# Contributing to LzCode

We want to make it easy for you to contribute to LzCode. Here are the most common type of changes that get merged:
...
- [`help wanted`](https://github.com/ingbyr/lzcode/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/ingbyr/lzcode/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/ingbyr/lzcode/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/ingbyr/lzcode/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
By default, `bun dev` runs LzCode in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run LzCode in the root of the opencode repo itself:
...
- Core pieces:
  - `packages/opencode`: LzCode core business logic & server.
...
To start the LzCode headless API server:
...
1. **First, start the LzCode server** (see [Running the API Server](#running-the-api-server) section above)
...
The most reliable way to debug LzCode is to run it manually in a terminal via `bun run --inspect=<url> dev ...` and attach
your debugger via that URL. Other methods can result in breakpoints being mapped incorrectly, at least in VSCode (YMMV).

Caveats:

- If you want to run the LzCode TUI and have breakpoints triggered in the server code, you might need to run `bun dev spawn` instead of
...
- The same problem arises when running LzCode in the VSCode `JavaScript Debug Terminal`
...
For net-new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach (optional), and why it belongs in LzCode. The core team will help decide whether it should move forward; please wait for that approval instead of opening a feature PR directly.
