...
nix run nixpkgs#opencode           # o github:ingbyr/lzcode para la rama dev más reciente
```
...
LzCode también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/ingbyr/lzcode/releases) o desde [opencode.ai/download](https://opencode.ai/download).
...
LzCode incluye dos agents integrados que puedes alternar con la tecla `Tab`.
...
Para más información sobre cómo configurar LzCode, [**ve a nuestra documentación**](https://opencode.ai/docs).

### Contribuir

Si te interesa contribuir a LzCode, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Construyendo sobre LzCode

Si estás trabajando en un proyecto relacionado con LzCode y usas "opencode" como parte del nombre; por ejemplo, "opencode-dashboard" u "opencode-mobile", agrega una nota en tu README para aclarar que no está construido por el equipo de LzCode y que no está afiliado con nosotros de ninguna manera.
...
- 100% open source
- No está acoplado a ningún proveedor. Aunque recomendamos los modelos que ofrecemos a través de [LzCode Zen](https://opencode.ai/zen); LzCode se puede usar con Claude, OpenAI, Google o incluso modelos locales. A medida que evolucionan los modelos, las brechas se cerrarán y los precios bajarán, por lo que ser agnóstico al proveedor es importante.
- Soporte LSP listo para usar
- Un enfoque en la TUI. LzCode está construido por usuarios de neovim y los creadores de [terminal.shop](https://terminal.shop); vamos a empujar los límites de lo que es posible en la terminal.
- Arquitectura cliente/servidor. Esto, por ejemplo, permite ejecutar LzCode en tu computadora mientras lo controlas de forma remota desde una app móvil. Esto significa que el frontend TUI es solo uno de los posibles clientes.
