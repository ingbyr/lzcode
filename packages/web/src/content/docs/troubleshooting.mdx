...
To debug issues with LzCode, start by checking the logs and local data it stores on disk.
...
LzCode Desktop runs a local LzCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `LzCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
LzCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so LzCode can rebuild it.

1. Quit LzCode Desktop completely.
...
3. Restart LzCode Desktop.

---

### Fix server connection issues

LzCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, LzCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with LzCode's features.

---

### Notifications not showing

LzCode Desktop only shows system notifications when:

- notifications are enabled for LzCode in your OS settings, and
...
1. Quit LzCode Desktop.
2. Find and delete these files (they live in the LzCode Desktop app data directory):
...
If you're experiencing issues with LzCode:
