---
title: Supporto ACP
description: Usa LzCode in qualunque editor compatibile con ACP.
---

LzCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare LzCode via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia LzCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'LzCode' nel selettore agenti di AI Chat.
...
Per usare LzCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare LzCode come agente ACP per la chat.
...
LzCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione LzCode
