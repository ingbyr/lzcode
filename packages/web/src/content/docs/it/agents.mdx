...
In LzCode esistono due tipi di agenti: agenti primari e subagenti.
...
LzCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
LzCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

LzCode include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, LzCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di LzCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione LzCode usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/ingbyr/lzcode).
:::
