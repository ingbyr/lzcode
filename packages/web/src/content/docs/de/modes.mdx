...
:::caution
Modi werden jetzt über die Option `agent` in der LzCode-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in LzCode können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die LzCode-Konfiguration.
...
LzCode verfügt über zwei integrierte Modi.
...
Wenn keine Temperatur angegeben ist, verwendet LzCode modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale LzCode-Konfiguration als auch die projektspezifische Konfiguration.
