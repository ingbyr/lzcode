---
title: IDE
description: Die LzCode-Erweiterung für VS Code, Cursor und andere IDEs
---

LzCode lässt sich in VS Code, Cursor oder jede IDE integrieren, die ein Terminal unterstützt. Führen Sie einfach `opencode` im Terminal aus, um zu beginnen.

---

## Verwendung

- **Schnellstart**: Verwenden Sie `Cmd+Esc` (Mac) oder `Ctrl+Esc` (Windows/Linux), um LzCode in einer geteilten Terminalansicht zu öffnen, oder fokussieren Sie eine dieser Terminalsitzung, falls bereits eine ausgeführt wird.
- **Neue Sitzung**: Verwenden Sie `Cmd+Shift+Esc` (Mac) oder `Ctrl+Shift+Esc` (Windows/Linux), um eine neue LzCode-Terminalsitzung zu starten, auch wenn eine bereits geöffnet ist. Sie können auch auf die LzCode-Schaltfläche im UI klicken.
- **Kontextbewusstsein**: Teilen Sie Ihre aktuelle Auswahl oder Registerkarte automatisch mit LzCode.
...
So installieren Sie LzCode in VS Code und beliebten Forks wie Cursor, Windsurf, VSCodium:
...
Suchen Sie im Extension Marketplace nach **LzCode** und klicken Sie auf **Installieren**.
