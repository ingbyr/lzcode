...
Es gibt zwei Arten von Agenten in LzCode; Primäragenten und Subagenten.
...
LzCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet LzCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale LzCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer LzCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenCode Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/ingbyr/lzcode).
:::
