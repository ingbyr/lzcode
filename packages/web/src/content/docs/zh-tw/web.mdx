---
title: Web
description: 在瀏覽器中使用 LzCode。
---

LzCode 可以作為 Web 應用程式在瀏覽器中執行，無需終端機即可獲得同樣強大的 AI 編碼體驗。
...
這會在 `127.0.0.1` 上啟動一個本地伺服器，使用隨機可用連接埠，並自動在預設瀏覽器中開啟 LzCode。
...
預設情況下，LzCode 會選擇一個可用連接埠。你也可以指定連接埠：
...
預設情況下，伺服器繫結到 `127.0.0.1`（僅限本地存取）。要使 LzCode 在網路中可存取：

```bash
opencode web --hostname 0.0.0.0
```

使用 `0.0.0.0` 時，LzCode 會同時顯示本地位址和網路位址：
