...
LzCode Zen — это список моделей, предоставленный командой opencode, которые были
протестировано и проверено на хорошую работу с opencode. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с LzCode Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `LzCode Zen` и перейдите по адресу [opencode.ai/auth](https://opencode.ai/zen).
...
LzCode Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой opencode, которые были
протестированы и проверены на хорошую работу с opencode.

1. Запустите команду `/connect` в TUI, выберите `LzCode Go` и перейдите по адресу [opencode.ai/auth](https://opencode.ai/zen).
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `LzCode`, Области применения: `api`
...
LzCode Zen — это список протестированных и проверенных моделей, предоставленный командой opencode. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>LzCode Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **LzCode Zen**.
