...
LzCode Go je povoljna pretplata od **$10/mjesečno** koja vam daje pouzdan pristup popularnim otvorenim modelima kodiranja.

:::note
LzCode Go je trenutno u beta fazi.
:::

Go funkcioniše kao bilo koji drugi pružatelj usluga u LzCode-u. Pretplatite se na LzCode Go i
...
:::tip
Testirali smo odabranu grupu modela i pružatelja usluga koji dobro rade sa LzCode-om.
:::
...
LzCode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako to funkcioniše

LzCode Go funkcioniše kao bilo koji drugi pružatelj usluga u LzCode-u.

1. Prijavite se na **<a href={console}>LzCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite `/connect` komandu u TUI-ju, odaberite `LzCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih putem Go.

:::note
Samo jedan član po radnom prostoru se može pretplatiti na LzCode Go.
:::
...
LzCode Go uključuje sljedeća ograničenja:
...
LzCode Go je pretplatnički plan od **$10/mjesečno**. Ispod su cijene **po 1M tokena**.
...
[Model id](/docs/config/#models) u vašoj LzCode konfiguraciji
...
4. Nemamo **zaključavanja (lock-in)** omogućavajući vam da koristite bilo kojeg drugog pružatelja usluga sa LzCode-om također.
