---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u LzCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LzCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj LzCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem LzCode
config.
...
LzCode Zen je lista modela koje je obezbedio LzCode tim koji su bili
testirano i potvrđeno da dobro radi sa LzCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa LzCode Zen.
:::
...
Radi kao i svaki drugi provajder u LzCode i potpuno je opcionalan za korištenje.

---

## LzCode Go

LzCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža LzCode tim i koji su testirani i verificirani da dobro rade s LzCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `LzCode Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s LzCode:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u LzCode.
:::
...
LzCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate LzCode
...
Za korištenje Google Vertex AI s LzCode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki LzCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki LzCode razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za LzCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s LzCode:
...
3. Kopirajte API ključ za korištenje u LzCode.
...
6. **Važno**: Prije upotrebe modela oblaka u LzCode, morate lokalno povući informacije o modelu:
...
LzCode Zen je lista testiranih i verifikovanih modela koju je obezbedio LzCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>LzCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **LzCode Zen**.

```txt
   /connect
```

3. Unesite svoj LzCode API ključ.
...
Polja `limit` omogućavaju LzCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite LzCode konfiguraciju i:
