---
title: CLI
description: LzCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LzCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa LzCode.
...
Pokrenite LzCode terminalski korisnički interfejs.
...
LzCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte LzCode agentima.
...
Priključite terminal na već pokrenut LzCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim LzCode backend-om. Na primjer:
...
LzCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se LzCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u LzCode.
...
Pokrenite LzCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| `--attach`   |        | Priključite na pokrenuti LzCode server (npr. http://localhost:4096)    |
...
Pokrenite LzCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti LzCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše LzCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili LzCode dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili LzCode dijeljenog URL-a.
...
Pokrenite LzCode headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup LzCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte LzCode i uklonite sve povezane datoteke.
...
Ažurira LzCode na najnoviju verziju ili određenu verziju.
...
LzCode CLI prihvata sljedeće globalne zastavice.
...
LzCode se može konfigurirati pomoću varijabli okruženja.
