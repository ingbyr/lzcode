---
title: LzCode Zen
description: LzCode によって提供されるモデルの厳選されたリスト。
---
...
LzCode Zen は、LzCode チームによって提供される、テストおよび検証されたモデルのリストです。

:::note
LzCode Zen は現在ベータ版です。
:::
Zen は LzCode の他のプロバイダーと同様に機能します。 LzCode Zen にログインすると、API キーを取得できます。これは **完全にオプション** であり、LzCode を使用するために使用する必要はありません。
...
:::tip
LzCode で適切に動作する、厳選されたモデルとプロバイダーのグループをテストしました。
:::
...
LzCode Zen は、これらのモデルへのアクセスを可能にする AI ゲートウェイです。

---

## 仕組み

LzCode Zen は、LzCode の他のプロバイダーと同様に機能します。

1. **<a href={console}>LzCode Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、LzCode Zen を選択して API キーを貼り付けます。
...
LzCode 設定の [model id](/docs/config/#models)
...
- MiniMax M2.5 Free は期間限定で LzCode で利用可能です。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
- Big Pickle は、期間限定で LzCode で無料で利用できるステルスモデルです。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
...
4. 他のコーディングエージェントとの併用を許可することで、**ロックイン**がなくなります。また、常に LzCode で他のプロバイダーも使用できるようにします。
