---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i LzCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LzCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i LzCode-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i LzCode
konfig.
...
LzCode Zen er en liste over modeller levert av LzCode-teamet som har vært
testet og verifisert for å fungere godt med LzCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med LzCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `LzCode Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i LzCode og er helt valgfritt å bruke.

---

## LzCode Go

LzCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av LzCode-teamet som har vært
testet og verifisert for å fungere godt med LzCode.

1. Kjør kommandoen `/connect` i TUI, velg `LzCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med LzCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører LzCode:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at LzCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom LzCode-konfigurasjonen.
...
for å lagre token i LzCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
LzCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse LzCode
...
Slik bruker du GitHub Copilot-abonnementet med LzCode:
...
Slik bruker du Google Vertex AI med LzCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører LzCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom LzCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver LzCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver LzCode-samtale oppført som en separat økt.
...
Du kan konfigurere LzCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere LzCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere LzCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for LzCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med LzCode:
...
3. Kopier API-nøkkelen for bruk i LzCode.
...
6. **Viktig**: Før du bruker skymodeller i LzCode, må du hente modellinformasjonen lokalt:
...
LzCode Zen er en liste over testede og verifiserte modeller levert av LzCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>LzCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **LzCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn LzCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom LzCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom LzCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LzCode:
...
Du kan også tilpasse modeller gjennom LzCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med LzCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar LzCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk LzCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i LzCode-konfigurasjonen.
