...
Det er to typer agenter i LzCode; primære agenter og underagenter.
...
LzCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
LzCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

LzCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Hvis ingen temperatur er spesifisert, bruker LzCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale LzCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i LzCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCode Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/ingbyr/lzcode).
:::
