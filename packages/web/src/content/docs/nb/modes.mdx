...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i LzCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i LzCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom LzCode-konfigurasjonen.
...
LzCode kommer med to innebygde moduser.
...
Hvis ingen temperatur er spesifisert, bruker LzCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale LzCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
