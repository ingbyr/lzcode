...
LzCode Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
LzCode Go is currently in beta.
:::

Go works like any other provider in LzCode. You subscribe to LzCode Go and
...
LzCode Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

LzCode Go works like any other provider in LzCode.

1. You sign in to **<a href={console}>LzCode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `LzCode Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to LzCode Go.
:::
...
LzCode Go includes the following limits:
...
4. Have **no lock-in** by allowing you to use any other provider with LzCode as well.
