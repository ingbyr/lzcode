---
title: CLI
description: Opcje i polecenia wiersza poleceń LzCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI LzCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
opencode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać LzCode programowo.
...
Uruchamia interfejs terminalowy (TUI) LzCode.
...
Interfejs CLI LzCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami LzCode.
...
Dołącz terminal do już działającego serwera backendu LzCode uruchomionego za pomocą `serve` lub `web`.

```bash
opencode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem LzCode. Na przykład:
...
LzCode integruje się z wieloma dostawcami. Możesz użyć `opencode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Podczas uruchamiania LzCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w LzCode.
...
Uruchom LzCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
| `--attach`   |       | Dołącz do działającego serwera LzCode (np. http://localhost:4096)       |
...
Uruchom serwer LzCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
opencode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API LzCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Pokaż statystyki wykorzystania tokenów i sesji LzCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania LzCode.
...
Uruchom serwer LzCode z interfejsem internetowym.

```bash
opencode web
```

Uruchamia to serwer HTTP i udostępnia LzCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Odinstaluj LzCode i usuń wszystkie powiązane pliki.
...
Aktualizuj LzCode do najnowszej lub określonej wersji.
...
Interfejs CLI LzCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
LzCode można skonfigurować za pomocą zmiennych środowiskowych.
