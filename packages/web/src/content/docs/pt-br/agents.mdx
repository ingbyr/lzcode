...
O ID do modelo em sua configuração do opencode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [LzCode Zen](/docs/zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/ingbyr/lzcode).
:::
