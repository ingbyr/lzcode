...
description: Brug LzCode i enhver ACP-kompatibel editor.
---

LzCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge LzCode via ACP, konfigurer din editor til at køre kommandoen `opencode acp`.

Kommandoen starter LzCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'LzCode'-agent i AI Chat-agentvælgeren.
...
For at bruge LzCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge LzCode som ACP-agent til chat.
...
LzCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din LzCode-konfiguration
