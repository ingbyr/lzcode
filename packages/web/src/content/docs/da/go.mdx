...
LzCode Go er et billigt **$10/måned** abonnement, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

:::note
LzCode Go er i øjeblikket i beta.
:::

Go fungerer ligesom enhver anden udbyder i LzCode. Du abonnerer på LzCode Go og får din API-nøgle. Det er **helt valgfrit**, og du behøver ikke at bruge det for at bruge LzCode.
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med LzCode.
:::
...
LzCode Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

LzCode Go fungerer ligesom enhver anden udbyder i LzCode.

1. Du logger ind på **<a href={console}>LzCode Zen</a>**, abonnerer på Go, og kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `LzCode Go`, og indsætter din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over modeller, der er tilgængelige gennem Go.

:::note
Kun ét medlem pr. workspace kan abonnere på LzCode Go.
:::
...
LzCode Go inkluderer følgende grænser:
...
LzCode Go er en **$10/måned** abonnementsplan. Nedenfor er priserne **pr. 1M tokens**.
...
[Model-id'et](/docs/config/#models) i din LzCode-konfiguration bruger formatet `opencode-go/<model-id>`. For eksempel, for Kimi K2.5, ville du bruge `opencode-go/kimi-k2.5` i din konfiguration.
...
4. Have **ingen lock-in** ved at tillade dig også at bruge enhver anden udbyder med LzCode.
