---
title: CLI
description: LzCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LzCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med LzCode programmatisk.
...
Start LzCode terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende LzCode backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern LzCode backend. For eksempel:
...
LzCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når LzCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i LzCode.
...
Start en hovedløs LzCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator LzCode sessionsholder.
...
Liste over alle LzCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine LzCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller LzCode del URL.
...
Start en hovedløs LzCode-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til LzCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller LzCode og fjern alle relaterede filer.
...
Opdaterer LzCode til den nyeste version eller en specifik version.
...
LzCode kan konfigureres ved hjælp af miljøvariabler.
