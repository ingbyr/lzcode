...
LzCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan **aylık 10$** tutarında düşük maliyetli bir aboneliktir.

:::note
LzCode Go şu anda beta aşamasındadır.
:::

Go, LzCode içindeki diğer sağlayıcılar gibi çalışır. LzCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve LzCode'u kullanmak için buna ihtiyacınız yoktur.
...
:::tip
LzCode ile iyi çalışan seçkin bir model ve sağlayıcı grubunu test ettik.
:::
...
LzCode Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır

LzCode Go, LzCode'daki diğer herhangi bir sağlayıcı gibi çalışır.

1. **<a href={console}>LzCode Zen</a>**'de oturum açın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `LzCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanılabilen modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Çalışma alanı başına yalnızca bir üye LzCode Go'ya abone olabilir.
:::
...
LzCode Go aşağıdaki sınırları içerir:
...
LzCode Go, **aylık 10$** tutarında bir abonelik planıdır. Aşağıda **1M token başına** fiyatlar yer almaktadır.
...
LzCode yapılandırmanızdaki [model kimliği](/docs/config/#models), `opencode-go/<model-id>` biçimini kullanır. Örneğin, Kimi K2.5 için yapılandırmanızda `opencode-go/kimi-k2.5` kullanırsınız.
...
LzCode Go'yu şu amaçlarla oluşturduk:
...
4. LzCode ile başka herhangi bir sağlayıcıyı kullanmanıza da izin vererek **kilitlenmeyi önlemek**.
