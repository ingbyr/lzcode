...
describe("tool.skill", () => {
  test("description lists skill location URL", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const skillDir = path.join(dir, ".lzcode", "skill", "tool-skill")
        await Bun.write(
    ...
    await using tmp = await tmpdir({
      ...
      init: async (dir) => {
        for (const [name, description] of [
          ...
          const skillDir = path.join(dir, ".lzcode", "skill", name)
          await Bun.write(
            path.join(skillDir, "SKILL.md"),
            `---
name: ${name}
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const dir = path.join(tmp.path, ".lzcode", "skill", "tool-skill")
