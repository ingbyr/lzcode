...
afterEach(async () => {
  ...
  await fs.rm(path.join(Global.Path.config, "lzcode.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "lzcode.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(Global.Path.config, "lzcode.json"),
        JSON.stringify(
          {
...
test("migrates tui-specific keys from opencode.json when tui.json does not exist", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "lzcode.json")))
...
test("skips migration when opencode.jsonc is syntactically invalid", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const source = await Filesystem.readText(path.join(tmp.path, "lzcode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "lzcode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "lzcode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "lzcode.json")
...
test("migrates legacy tui keys across multiple opencode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "lzcode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "lzcode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(customDir, "lzcode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENCODE_CONFIG = path.join(customDir, "lzcode.json")
    },
  })
