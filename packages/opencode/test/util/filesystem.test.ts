...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "lzcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "lzcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "lzcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "lzcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["lzcode.json", "lzcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "lzcode.json"),
        path.join(tmp.path, "lzcode.jsonc"),
        path.join(project, "lzcode.json"),
        path.join(project, "lzcode.jsonc"),
      ])
    })
  })
