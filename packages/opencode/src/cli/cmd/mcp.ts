    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in lzcode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .lzcode/ subdirectory too)
  const candidates = [path.join(baseDir, "lzcode.json"), path.join(baseDir, "lzcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".lzcode", "lzcode.json"), path.join(baseDir, ".lzcode", "lzcode.jsonc"))
  }
  ...
  // Default to lzcode.json if none exist
  return candidates[0]
}
