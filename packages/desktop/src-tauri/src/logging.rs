...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("lzcode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("lzcode_lib=debug,lzcode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("lzcode_lib=info,lzcode_desktop=info,sidecar=info")
        }
    });
